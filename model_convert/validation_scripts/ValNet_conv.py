...
def main():
    ...
    dummy_x = torch.randn(1, 1, 5, 5)
    #dummy_h = torch.randn(1, 1, 3)
    #dummy_input = (dummy_x, dummy_h)
    current_date = date.today()
    filename = "ValNet_Conv"

    onnx_filename = f"{filename}_{current_date}.onnx"
   ...
   # Export to CoreML
    ...
    mlmodel_filename = f"{filename}_{current_date}.mlmodel"
    ...
    test_x = torch.randn(1, 1, 5, 5) # different time_dim to test dynamic seq length
    ...
    #Predict with Coreml
    coreml_output_dict = mlmodel.predict({'input':test_x_np}, useCPUOnly=True)#, 'hidden_in': test_h_np}, useCPUOnly=True)
    ...
    np.testing.assert_allclose(to_numpy(torch_output), ort_output, rtol=1e-5, atol=0)
    print("Torch and ONNX output predictions match!\n")
    #np.testing.assert_allclose(to_numpy(torch_h), ort_h, rtol=1e-03, atol=1e-05)
    #print("Torch and ONNX output hidden states match!\n")

    
    np.testing.assert_allclose(to_numpy(torch_output), coreml_output, rtol=1e-5, atol=0)
    print("Torch and Coreml output predictions match!\n")
    #np.testing.assert_allclose(to_numpy(torch_h), coreml_h, rtol=1e-03, atol=1e-05)
    #print("Torch and Coreml hidden states match!\n")


    np.testing.assert_allclose(coreml_output, ort_output, rtol=1e-5, atol=0)
    print("ONNX and Coreml predictions match!\n")
    #np.testing.assert_allclose(coreml_h, ort_h, rtol=1e-03, atol=1e-05)
    #print("Torch and Coreml hidden states match!\n")
