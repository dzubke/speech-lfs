...
def main():
   ...
   # Export to CoreML
    ...
    # Predict with Torch
    torch_output, torch_h = model(test_x, test_h)

    # Predict with Onnx
    ort_session = onnxruntime.InferenceSession(onnx_path)
    ort_inputs = {ort_session.get_inputs()[0].name: test_x_np, ort_session.get_inputs()[1].name: test_h_np}
    ort_output, ort_h = ort_session.run(None, ort_inputs)

    #Predict with Coreml
    coreml_output_dict = mlmodel.predict({'input':test_x_np, 'hidden_in': test_h_np}, useCPUOnly=True) 
    coreml_output, coreml_h  = coreml_output_dict['output'], coreml_output_dict['hidden_out'] 
    ...
    torch_onnx_rtol = 1e-1
    torch_onnx_atol = 0

    # Compare Torch and ONNX predictions
    np.testing.assert_allclose(to_numpy(torch_output), ort_output, rtol=torch_onnx_rtol, atol=torch_onnx_atol)
    print("Torch and ONNX output predictions match!")
    np.testing.assert_allclose(to_numpy(torch_h), ort_h, rtol=torch_onnx_rtol, atol=torch_onnx_atol)
